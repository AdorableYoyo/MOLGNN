...
# Degine a GIN and GAE model to train the GIN and GAE at the same time.
class GIN_GAE(nn.Module):
    """GIN_GAE model"""
    ...
    def forward(self, g, h):
        ...
        for i, h in enumerate(hidden_rep):
            ...
            adj_rec = self.decoder(score_over_layer_gae)
        ...
        return adj_rec, score_over_layer_classification, fingerprint_rec
...
class FingerprintDecoder(nn.Module):
    def __init__(self, n_in, n_out, dropout=0.1):
        # n_in is the same as the hidden representation of the GAE.
    ...
    def forward(self, x):
        ...
        x = self.fc2(x)
        # this is different to Yang's network as we are directly processing based on the N*hidden output of GAE
        # x = torch.mean(x, -2)
        return x
