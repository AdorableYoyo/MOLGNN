...
def main(args):
    ...
    tflogs = (
        args.tflog
        + "/"
        + args.data_splitting_method
        + args.experiment_repeat_id
        + "/"
    ...
    if args.dataset in _DeepChemDatasets:
        ...
        deepchem_fingerprint_datapath = (
            args.deepchem_datapath + args.dataset + "FP" + "/"
        )
        assert os.path.exists(
            deepchem_fingerprint_datapath
        ), "currently version code requires the exists of the fingerprint data"
        ...
        )  # the reason that we need the full datasets information as we need align features from all datasets to do the one hot encoding
    ...
    trainloader, validloader, testloader = GraphDataLoaderSplit(
        ...
        main_dataset=args.dataset,
        USING_CORRECT_scaffold_split_implementation=False,
    ).get_data_loader()
    ...
    for epoch, _, _ in zip(tbar, vbar, lrbar):
        ...
        if args.train_gae and args.gae_train_method == "gradual_unfreeze":
            ...
            stage_key = {0: "01234", 1: "0123", 2: "012", 3: "01", 4: "0"}
            learning_rates_stages = {
                0: args.lr,
                1: args.lr,
                2: args.lr / 2.0,
                3: args.lr / 2.0,
            ...
            if epoch_finetune >= 0 and (epoch_finetune % 25 == 0):
                ...
                if 0 <= frozen_stage <= 4:
                    ...
                    optimizer = optim.Adam(
                        ...
                        lr=learning_rates_stages[frozen_stage],
                    )
        ...
        writer.add_scalar(
            "Loss/train_loss_gae_original_WHOLE", train_loss_gae_original, epoch
        )
        writer.add_scalar(
            "Loss/train_loss_classification_original_WHOLE",
            train_loss_classification_original,
            epoch,
        )
        writer.add_scalar(
            "Loss/train_loss_fingerprint_original_WHOLE",
            train_loss_fingerprint_original,
            epoch,
        )
        writer.add_scalar(
            "Loss/train_loss_gae_weighted_WHOLE", train_loss_gae_weighted, epoch
        )
        writer.add_scalar(
            "Loss/train_loss_classification_weighted_WHOLE",
            train_loss_classification_weighted,
            epoch,
        )
        writer.add_scalar(
            "Loss/train_loss_fingerprint_weighted_WHOLE",
            train_loss_fingerprint_weighted,
            epoch,
        )
        ...
        if validloader is not None:
            ...
            writer.add_scalar(
                "Loss/valid_loss_gae_original_WHOLE", valid_loss_gae_original, epoch
            )
            writer.add_scalar(
                "Loss/valid_loss_classification_original_WHOLE",
                valid_loss_classification_original,
                epoch,
            )
            writer.add_scalar(
                "Loss/valid_loss_fingerprint_original_WHOLE",
                valid_loss_fingerprint_original,
                epoch,
            )
            writer.add_scalar(
                "Loss/valid_loss_gae_weighted_WHOLE", valid_loss_gae_weighted, epoch
            )
            writer.add_scalar(
                "Loss/valid_loss_classification_weighted_WHOLE",
                valid_loss_classification_weighted,
                epoch,
            )
            writer.add_scalar(
                "Loss/valid_loss_fingerprint_weighted_WHOLE",
                valid_loss_fingerprint_weighted,
                epoch,
            )
            ...
            writer.add_scalar(
                "roc_score_micro/valid_WHOLE", valid_roc_score_micro, epoch
            )
            writer.add_scalar("ap_score_micro/valid_WHOLE", valid_ap_score_micro, epoch)
        # test
        writer.add_scalar("Loss/test_WHOLE", test_loss, epoch)
        writer.add_scalar(
            "Loss/test_loss_gae_original_WHOLE", test_loss_gae_original, epoch
        )
        writer.add_scalar(
            "Loss/test_loss_classification_original_WHOLE",
            test_loss_classification_original,
            epoch,
        )
        writer.add_scalar(
            "Loss/test_loss_fingerprint_original_WHOLE",
            test_loss_fingerprint_original,
            epoch,
        )
        writer.add_scalar(
            "Loss/test_loss_gae_weighted_WHOLE", test_loss_gae_weighted, epoch
        )
        writer.add_scalar(
            "Loss/test_loss_classification_weighted_WHOLE",
            test_loss_classification_weighted,
            epoch,
        )
        writer.add_scalar(
            "Loss/test_loss_fingerprint_weighted_WHOLE",
            test_loss_fingerprint_weighted,
            epoch,
        )
        ...
        if (
            (not args.train_gae)
            or (args.train_gae and epoch >= args.pretrain_epochs)
            or (args.train_gae and args.gae_train_method == "static_fusing")
        ):
            if not args.train_gae or (
                args.train_gae and args.gae_train_method == "static_fusing"
            ):
                epoch_ = epoch
            ...
            writer.add_scalar(
                "Loss/train_loss_gae_original", train_loss_gae_original, epoch_
            )
            writer.add_scalar(
                "Loss/train_loss_classification_original",
                train_loss_classification_original,
                epoch_,
            )
            writer.add_scalar(
                "Loss/train_loss_fingerprint_original",
                train_loss_fingerprint_original,
                epoch_,
            )
            writer.add_scalar(
                "Loss/train_loss_gae_weighted", train_loss_gae_weighted, epoch_
            )
            writer.add_scalar(
                "Loss/train_loss_classification_weighted",
                train_loss_classification_weighted,
                epoch_,
            )
            writer.add_scalar(
                "Loss/train_loss_fingerprint_weighted",
                train_loss_fingerprint_weighted,
                epoch_,
            )
            ...
            if validloader is not None:
                ...
                writer.add_scalar(
                    "Loss/valid_loss_gae_original", valid_loss_gae_original, epoch_
                )
                writer.add_scalar(
                    "Loss/valid_loss_classification_original",
                    valid_loss_classification_original,
                    epoch_,
                )
                writer.add_scalar(
                    "Loss/valid_loss_fingerprint_original",
                    valid_loss_fingerprint_original,
                    epoch_,
                )
                writer.add_scalar(
                    "Loss/valid_loss_gae_weighted", valid_loss_gae_weighted, epoch_
                )
                writer.add_scalar(
                    "Loss/valid_loss_classification_weighted",
                    valid_loss_classification_weighted,
                    epoch_,
                )
                writer.add_scalar(
                    "Loss/valid_loss_fingerprint_weighted",
                    valid_loss_fingerprint_weighted,
                    epoch_,
                )
                ...
                writer.add_scalar(
                    "roc_score_micro/valid", valid_roc_score_micro, epoch_
                )
                writer.add_scalar("ap_score_micro/valid", valid_ap_score_micro, epoch_)
            # test
            writer.add_scalar("Loss/test", test_loss, epoch_)
            writer.add_scalar(
                "Loss/test_loss_gae_original", test_loss_gae_original, epoch_
            )
            writer.add_scalar(
                "Loss/test_loss_classification_original",
                test_loss_classification_original,
                epoch_,
            )
            writer.add_scalar(
                "Loss/test_loss_fingerprint_original",
                test_loss_fingerprint_original,
                epoch_,
            )
            writer.add_scalar(
                "Loss/test_loss_gae_weighted", test_loss_gae_weighted, epoch_
            )
            writer.add_scalar(
                "Loss/test_loss_classification_weighted",
                test_loss_classification_weighted,
                epoch_,
            )
            writer.add_scalar(
                "Loss/test_loss_fingerprint_weighted",
                test_loss_fingerprint_weighted,
                epoch_,
            )
        ...
        if args.filename:
            with open(args.filename, "a") as f:
                ...
                if validloader is None:
                    f.write(
                        "%f %f %f %f" % (train_loss, train_acc, test_loss, test_acc)
                    )
                    f.write("\n")
                    f.write(
                        "%f %f %f %f"
