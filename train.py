...
def train(
    ...
    for pos, (graphs, labels, fingerprints) in zip(bar, trainloader):
        ...
        adj_rec, mu, logvar, score_over_layer_classification, fingerprint_rec = net(
            graphs, feat
        )
        print(
            "fingerprint_gt.shape, fingerprint_rec.shape",
            fingerprint_gt.shape,
            fingerprint_rec.shape,
        )
        ...
        # print("score_over_layer_classification.size(), labels.size()",
        # score_over_layer_classification.size(), labels.size())
        mask = labels != -1
        if torch.sum(~mask) > 0:
