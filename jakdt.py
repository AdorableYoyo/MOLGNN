...
#FEATURE_NAMES = ['atom_type', 'degree', 'formal_charge', 'hybridization_type', 'aromatic', 'chirality_type']
...
_MultipleLabelDatasets = {'Sider':27,
                          'ICLR2020_Sider':27, 
                          'ICLR2020_ClinTox':2,
                          'ClinTox_twoLabel':2}
"""
...
Numbers in the parethesis are theoretical max number of types. In our dataset, the number is very likely to be smaller. Need to be
checked.

"""
#ONEHOTENCODING = [0, 1,2,3,5] 
ONEHOTENCODING_CODEBOOKS = {}
# when the graph label is not available, we set a dummy label. Although the loss contributed by the dummy label will 
# not have an influence to the final performance as the classification loss will be multipled by 0, in order to 
# make the loss caculation process is running well, we set this dummy label as a valid value. For example, the 
...
class GINDataset(object):
    ...
    def __init__(self, 
        ...
        self.FEATURE_NAMES = ['atom_type', 'degree', 'formal_charge', 'hybridization_type', 'aromatic', 'chirality_type']
    ...
    def _get_onehot_encoding_features(self):
        ...
        if ONEHOTENCODING_CODEBOOKS:
            ...
            node_attributes_filenames = [self.datapath+dataset_name+'/'+dataset_name+'_node_attributes.txt' 
            ...
            for i, filename in enumerate(node_attributes_filenames):
                ...
                this_node_attributes = [[float(l) for l in line.split(',')] for line in lines]
            ...
            ONEHOTENCODING_CODEBOOKS.update({feature_name:sorted(node_attributes_cnt[feature_name].keys()) 
                                             for feature_name in self.FEATURE_NAMES})
        node_attributes_we_needed_filename = self.datapath+self.dataset+'/'+self.dataset+'_node_attributes.txt' 
        with open(node_attributes_we_needed_filename, "r") as f:
            lines = f.readlines()
        node_attributes_we_needed = [[float(l) for l in line.split(',')] for line in lines]
        node_attributes_one_hot = []
        #ONEHOTENCODING_CODEBOOKS = {'atom_type': [0.0, 1.0, 2.0, 3.0, 4.0, 7.0, 8.0, 14.0], 'degree': [1.0, 2.0, 3.0, 4.0], 'formal_charge': [-1.0, 0.0, 1.0], 'hybridization_type': [2.0, 3.0, 4.0], 'aromatic': [0.0, 1.0], 'chirality_type': [0.0, 1.0, 2.0]}
        for row in node_attributes_we_needed:
            ...
            for j, feature_val_before_onehot in enumerate(row):
                if j in self.ONEHOTENCODING:
                    ...
                    onehot_values[onehot_codebook.index(feature_val_before_onehot)] = 1.0
    ...
    def _get_graph_raw_data(self):
        ...
        with open(self.datapath+self.dataset+'/'+graph_indicator_filename, "r") as f:
            lines = f.readlines()
        graph_indicator = [int(line) for line in lines]
        
         # get graph labels
        if self.dataset in _LABEL_AVAILABLE_DATASETS:
            with open(self.datapath+self.dataset+'/'+graph_labels_filename, "r") as f:
                lines = f.readlines()
            graph_labels = [int(line) for line in lines]
            with open(self.datapath+self.dataset+'/'+graph_fingerprint_filename, "r") as f:
                lines = f.readlines()
        ...
        if self.featureencoding=='onehot':
            ...
            with open(self.datapath+self.dataset+'/'+node_attributes_filename, "r") as f:
                lines = f.readlines()
    ...
    def _load(self):
        ...
        A, graph_indicator, graph_labels, node_attributes, graph_fingerprint = self._get_graph_raw_data()
        ...
        if self.debug:print(f"(nodes_per_graph[:10]: {nodes_per_graph[:10]}")
        ...
        for graph_id, raw_label in enumerate(graph_labels):
            if self.debug:print("*"*20 + str(graph_id))
            m_edges = 0
            ...
            if self.debug:print(f"nodes_of_this_g, {nodes_of_this_g}")
            g = dgl.DGLGraph()
            ...
            while True:
                if raw_large_edges_idx>=len(A):break
                old_u, old_v = A[raw_large_edges_idx]
                if old_u>end_node_idx:
                    #assert old_u==end_node_idx+1, "old_u, {}, old_v {}, end_node_idx{}".format(old_u, old_v, end_node_idx)
                ...
                if self.debug:print('edge:', old_u, old_v, new_u, new_v)
        ...
        print(
            ...
            New Label Relabeled: %s   \n """ % (
                ...
                self.nlabel_dict, raw_label_dict, new_label_dict))

class DeepChemDataset(object):
    """Datasets for DeepChem
    DeepChem is already loaded by using PyTorch Geometric 
    the dataset from PyTorch Geometric is named dataset_pg. It has the format as [graph0, grpah1, graph2, graph3 ..]
    each graph is a Data() instance and the class Data is defined here:
    the fingerprint data is saved in dataset_fingerprint_pg. It has the format as [graph0, graph1, graph2, graph3 ...]
    The label for the dataset_fingerprint_pg is the fingerprint.
...
tensor([[ 0,  1,  1,  1,  2,  3,  3,  4,  4,  4,  5,  6,  6,  6,  7,  7,  8,  8,
    ...
    def __init__(self, 
                 ...
                 full_datasets=None,
                 debug=False,
                 main_dataset=None): # main_dataset is the dataset that we are used to train the graph classifier
        """Initialize the dataset."""
        self.graph_labels_available = True
        if dataset_pg is None:
            print("graph labels are not available for this dataset, set the self.dataset_pg the same as dataset_fingerprint_pg to keep the old framework working  ")
            self.graph_labels_available = False
            # warning, here by setting the dataset_pg to dataset_fingerprint_pg is only trying to make the old codebase working.
    ...
    def _get_ONEHOTENCODING_CODEBOOKS(self):
        ...
        print('the node feature size if we put all graphs node features together', features.shape)
    ...
    def _parse_dataset_pg(self):
        ...
        if self.featureencoding=='onehot':
            ...
            if self.debug:print("ONEHOTENCODING_CODEBOOKS", ONEHOTENCODING_CODEBOOKS)
        #assert len(self.dataset_pg) == len(self.dataset_fingerprint_pg), "ataset_pg should have the same size as the dataset_fingerprint_pg, we get dataset_pg size of {} dataset_fingerprint_pg size of {}".format(len(self.dataset_pg), len(self.dataset_fingerprint_pg))
        ...
        for data, data_fingerprint in zip(self.dataset_pg, self.dataset_fingerprint_pg):
            ...
            if self.graph_labels_available:
                if self.main_dataset in _MultipleLabelDatasets:
                    ...
                    assert len(label) == _MultipleLabelDatasets[self.main_dataset], "label dimension should match, len of label is {}".format(len(label)) 
                else:
                    label = int(data.y)
            else:
                if self.main_dataset in _MultipleLabelDatasets:
            ...
            edge_index_from_fp_data, feature_from_fingerprint_data, fingerprint = (
                data_fingerprint.edge_index, data_fingerprint.x.numpy(), data_fingerprint.y.numpy())
            fingerprint = np.array(fingerprint, dtype=float)
            # the original fingerprint has the shape of (a, 740), change it to shape of (740,)
            ...
            #np.savetxt(str(datasample_counter)+"feature_from_fp.txt",feature_from_fingerprint_data)
            assert feature.shape[0] == feature_from_fingerprint_data.shape[0],  "the feature shpae 0 (number of nodes in the graph) from those two datasets should be eqaul. sample index is {}".format(datasample_counter)
            assert np.array_equal(edge_index, edge_index_from_fp_data), "the feature from those two dataset should be eqaul"
            assert np.array_equal(feature, feature_from_fingerprint_data), "the feature from those two dataset should be eqaul {}".format(datasample_counter)
            ...
            u, v = edge_index
            if self.featureencoding=='onehot':
                if self.debug:print("before onehot encoding", feature)
                feature = self._get_onehot_encoding_features(feature)
                if self.debug:print("after onehot encoding", feature)
        ...
        print(
            ...
            Avg. of #Edges: %.2f \n """ % (
                self.N, self.gclasses, self.n, self.nclasses,
                self.dim_nfeats, self.m, self.eclasses, self.fingerprint_dim,
